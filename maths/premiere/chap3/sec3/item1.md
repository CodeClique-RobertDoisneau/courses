...
C’est comme se demander : “À quel moment la suite $(u_n)$ devient plus grande que 1 000 ?”
...
Pour calculer le seuil, le problème c’est que nous ne connaissons pas le rang $n
$ du terme que l’on veut calculer. Nous devons alors calculer tous les termes tant que la condition n’est pas vérifiée et dès que c’est le cas, nous avons donc obtenu le terme souhaité !

**Méthode** : 

* On initialise dans une variable le terme initiale ($u_0$)  
...
* Dans la boucle, on calcule le terme suivant dans la variable $u_n$  
...
Formule explicite : par exemple  $u_n = 3n + 4$  
```python
# --- INITIALISATION ---   
n = 0   
...
```python
	...
	n = 0				# Le terme dans la variable un_1 correspond au        
...
* Dans la boucle while, incrémenter de 1 la variable correspondant au rang pour que le rang corresponde à celui du terme calculé à la ligne précédente  
...
Exemple : On pose $u_{n+1} = 3u_n + 6$ pour tout $n$ entier naturel et $u_0 = 2$, déterminer à partir de quel rang $u_n$ est-il supérieur ou égale à 100, d’abord sans utiliser une fonction, puis en utilisant une fonction.   
```python
